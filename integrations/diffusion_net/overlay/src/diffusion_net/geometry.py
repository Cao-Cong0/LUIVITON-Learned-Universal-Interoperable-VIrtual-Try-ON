...
def normalize_positions(pos, body_pos=None, faces=None, method='mean', scale_method='max_rad'):
    ...
    if body_pos is None:
        ...
        if method == 'mean':
            # center using the average point position
            pos = (pos - torch.mean(pos, dim=-2, keepdim=True))
            body_pos = (body_pos - torch.mean(body_pos, dim=-2, keepdim=True))
            ...
            # body_bbox_min = torch.min(body_pos, dim=-2).values
            # body_bbox_max = torch.max(body_pos, dim=-2).values
            # body_center = (body_bbox_max + body_bbox_min) / 2.
            # body_pos -= body_center.unsqueeze(-2)

        else:
            raise ValueError("unrecognized method")
        
        combined_pos = torch.cat([body_pos, pos], dim=0)
        if scale_method == 'max_rad':
            scale = torch.max(norm(combined_pos), dim=-1, keepdim=True).values.unsqueeze(-1)
            combined_pos = combined_pos / scale
            pos = combined_pos[len(body_pos):]
