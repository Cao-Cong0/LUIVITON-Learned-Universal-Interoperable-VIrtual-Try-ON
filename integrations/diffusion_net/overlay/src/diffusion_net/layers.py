...
class SpatialPositionalEncoding(nn.Module):
    def __init__(self, max_freq_log2, num_freqs, embedding_dim):
        super(SpatialPositionalEncoding, self).__init__()

        # Generate frequency steps
        self.freqs = 2. ** torch.linspace(0, max_freq_log2, steps=num_freqs)
        self.embedding_dim = embedding_dim
        dimensions_per_axis = embedding_dim // 3 // 2  # divide by 3 axes, and each requires a sin and a cos
        self.dimensions_per_axis = dimensions_per_axis

    def forward(self, coordinates):
        # Ensure that freqs are on the same device as input coordinates
        freqs = self.freqs.to(coordinates.device)

        x, y, z = coordinates[..., 0], coordinates[..., 1], coordinates[..., 2]
        x_scaled = x.unsqueeze(-1) * freqs[:self.dimensions_per_axis]
        y_scaled = y.unsqueeze(-1) * freqs[:self.dimensions_per_axis]
        z_scaled = z.unsqueeze(-1) * freqs[:self.dimensions_per_axis]

        x_encoded = torch.cat((torch.sin(x_scaled), torch.cos(x_scaled)), dim=-1)
        y_encoded = torch.cat((torch.sin(y_scaled), torch.cos(y_scaled)), dim=-1)
        z_encoded = torch.cat((torch.sin(z_scaled), torch.cos(z_scaled)), dim=-1)

        # Concatenate along the last dimension and pad if necessary
        encoded = torch.cat((x_encoded, y_encoded, z_encoded), dim=-1)
        padding_needed = self.embedding_dim - encoded.shape[-1]
        if padding_needed > 0:
            padding = torch.zeros(*encoded.shape[:-1], padding_needed, device=coordinates.device)
...
class MiniMLP(nn.Sequential):
    ...
    def __init__(self, layer_sizes, dropout=False, activation=nn.ReLU, name="miniMLP"):
        ...
        for i in range(len(layer_sizes) - 1):
            ...
            # Optionally add dropout after layer normalization but before activation
            if dropout and i > 0:
                self.add_module(
                    name + "_mlp_layer_dropout_{:03d}".format(i),
                    nn.Dropout(p=.5)
...
class DiffusionNetBlock(nn.Module):
    ...
    def __init__(self, C_width, mlp_hidden_dims,
        ...
        # After diffusion
        self.layer_norm1 = nn.LayerNorm(C_width)

        # Before MLP
        self.layer_norm2 = nn.LayerNorm(self.MLP_C)
    ...
    def forward(self, x_in, mass, L, evals, evecs, gradX, gradY):
        ...
        x_diffuse = self.layer_norm1(x_diffuse)
        ...
        feature_combined = self.layer_norm2(feature_combined)
        # Apply the mlp
...
class DiffusionNet(nn.Module):
    ...
    def __init__(self, C_in, C_out, C_width=128, N_block=4, last_activation=None, outputs_at='vertices', mlp_hidden_dims=None, dropout=True, 
        ...
        self.increaser = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0),
            nn.Linear(32, C_width)
        )

        self.reducer = nn.Sequential(
            nn.Linear(C_width, 32),
            nn.ReLU(),
            nn.Dropout(0),
            nn.Linear(32, C_out)
        )
        ...
        # self.positionalencoding = SpatialPositionalEncoding(max_freq_log2=10, num_freqs=21, embedding_dim=self.C_width)


    def positional_encoding(self, positions, freqs, out_dim):
            ...
            padding_needed = out_dim - pts.shape[-1]
            if padding_needed > 0:
                padding = torch.zeros(*pts.shape[:-1], padding_needed, device=positions.device)
    ...
    def forward(self, x_in, mass, L=None, evals=None, evecs=None, gradX=None, gradY=None, edges=None, faces=None):
        ...
        if x_in.shape[-1] != self.C_in: # x_in shape [N,3]
        ...
        # pe = self.positionalencoding(x_in[0]).unsqueeze(0) # [1,N,C_width]

        # pe = self.positional_encoding(x_in[0], 10, self.C_width).unsqueeze(0) # [1,N,C_width]


        # Apply the first linear layer
        # x = self.first_lin(x_in) # [1,N,C_width], C_width=128
        x = self.increaser(x_in) # [1,N,C_width], C_width=128
        # x = x + pe # [1,N,C_width]
        ...
        # Apply the last linear layer, now x is [1,N,C_width]

        # x = x + pe  # [1,N,C_width]
        # x = self.last_lin(x)
        x = self.reducer(x) # [1,N,C_out]
