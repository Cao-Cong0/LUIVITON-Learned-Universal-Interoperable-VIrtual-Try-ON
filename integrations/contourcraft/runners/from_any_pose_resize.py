...
class MaterialConfig:
    ...
    density_min: float = 0.050022
    density_max: float = 0.050022
...
class Runner(nn.Module):
    def __init__(self, model: nn.Module, criterion_dict: Dict[str, nn.Module],
        ...
        self.model = model
        self.criterion_dict_hood = criterion_dict['ccraft']
        self.mcfg = mcfg
    ...
    def _rollout(self, sample, start_idx, n_steps, progressbar=False, bare=False, safecheck=True):
        ...
        for i in pbar:
            ...
            sample_step['cloth'].rest_pos = sample_step['cloth'].rest_pos[:, i, :].clone().to(torch.float32)
            sample_step['cloth'].v_mass = sample_step['cloth'].v_mass[:, i, :].clone().to(torch.float32)
            sample_step['cloth'].f_area = sample_step['cloth'].f_area[:, i, :].clone().to(torch.float32)
            sample_step['cloth'].Dm_inv = sample_step['cloth'].Dm_inv[:, i, :].clone().to(torch.float32)
    ...
    def _add_cloth_obj(self, sample):
        ...
        if sample['cloth']['rest_pos'].ndim == 2:
            sample = self.cloth_obj.set_batch(sample, overwrite_pos=self.mcfg.overwrite_pos_every_step)
        elif sample['cloth']['rest_pos'].ndim == 3:
            sample = self.cloth_obj.set_batch_frame_wise(sample, overwrite_pos=self.mcfg.overwrite_pos_every_step)
