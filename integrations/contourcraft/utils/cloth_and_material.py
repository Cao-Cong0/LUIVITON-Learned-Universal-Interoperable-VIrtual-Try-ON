...
class Cloth:
    ...
    def set_batch_frame_wise(self, batch, overwrite_pos=False):
        """Compute cloth properties for a frame-wise rest-position tensor."""
        new_examples = []
        for i in range(batch.num_graphs):
            ...
            f_area_frames = []
            dm_inv_frames = []
            for frame in range(example['cloth'].rest_pos.shape[1]):
                vertices = example['cloth'].rest_pos[:, frame, :]
                v_mass = self.make_v_mass(vertices, faces, density, vertices.device)

                if garment_name in self.cache and not overwrite_pos:
                    f_area = self.cache[garment_name]['f_area']
                    dm_inv = self.cache[garment_name]['Dm_inv']
                    connectivity = self.cache[garment_name]['f_connectivity']
                    connectivity_edges = self.cache[garment_name]['f_connectivity_edges']
                else:
                    f_area = self.make_f_area(vertices, faces, vertices.device)
                    dm_inv = self.make_Dm_inv(vertices, faces)
                    connectivity, connectivity_edges, _ = self.make_connectivity(faces)
                    self.cache[garment_name].update(
                        v_mass=v_mass,
                        f_area=f_area,
                        Dm_inv=dm_inv,
                        f_connectivity=connectivity,
                        f_connectivity_edges=connectivity_edges,
                    )

                v_mass_frames.append(v_mass)
                f_area_frames.append(f_area)
                dm_inv_frames.append(dm_inv)

            example['cloth'].v_mass = torch.stack(v_mass_frames).transpose(0, 1)
            example['cloth'].f_area = torch.stack(f_area_frames).transpose(0, 1)
            example['cloth'].Dm_inv = torch.stack(dm_inv_frames).transpose(0, 1)
            example['cloth'].f_connectivity = connectivity
            example['cloth'].f_connectivity_edges = connectivity_edges
            new_examples.append(example)

        return Batch.from_data_list(new_examples)
