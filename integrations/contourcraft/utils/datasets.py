...
def make_fromanypose_resize_dataloader(
    pose_sequence_type,
    pose_sequence_path,
    ...
    from datasets.from_any_pose_resize import Config as DatasetConfig
    from datasets.from_any_pose_resize import create as create_dataset
    from utils.arguments import DataConfig as DataloaderConfig
    from utils.dataloader import DataloaderModule

    config = DatasetConfig(
        pose_sequence_type=pose_sequence_type,
        pose_sequence_path=pose_sequence_path,
        garment_template_path=garment_template_path,
        scale_x=scale_x,
        scale_y=scale_y,
        ...
        restpos_scale=restpos_scale,
    )
    dataset = create_dataset(config)
    dataloader_config = DataloaderConfig(num_workers=0)
    return DataloaderModule(dataset, dataloader_config).create_dataloader()
