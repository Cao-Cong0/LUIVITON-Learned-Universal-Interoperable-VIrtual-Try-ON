...
class Config:
    ...
    num_frames: int = 15
    restpos_scale: bool = True  # Whether to scale the rest pose of the garment gradually
    scale_x: float = 1.0
    scale_y: float = 1.0
    scale_z: float = 1.0
...
class GarmentBuilder:
    ...
    def add_verts(self, sample: HeteroData, garment_dict: dict) -> HeteroData:
        ...
        if self.mcfg.restpos_scale:
            sample['cloth'].rest_pos = self.resize_restpos_gradually(pos[:, 0].clone())
        else:
            sample['cloth'].rest_pos = pos[:, 0].clone()
        

        return sample
    
    def resize_restpos_gradually(self, restpos: np.array) -> np.array:
        """
        ...
        N = self.mcfg.num_frames
        
        # # make scale gradually change from 1 to 3, using N steps
        # scale_x = np.linspace(1, self.mcfg.scale_x, N)  # Scale for x-axis
        # scale_y = np.linspace(1, self.mcfg.scale_y, N)  # Scale for y-axis, won't change the length of sleeve
        # scale_z = np.linspace(1, self.mcfg.scale_z, N)  # Scale for z-axis

        # adjust the scale from very beginning
        scale_x = np.full(N, self.mcfg.scale_x)
        scale_y = np.full(N, self.mcfg.scale_y)
        scale_z = np.full(N, self.mcfg.scale_z)

        scale = np.stack([scale_x, scale_y, scale_z], axis=1)  # shape (N, 3)
        
        mean = restpos.mean(axis=0, keepdims=True)
        restpos -= mean # shape (V, 3)
        # multiply every scale to restpos to get shape (N, V, 3)
        restpos = np.repeat(restpos[np.newaxis, :, :], N, axis=0) # shape (N, V, 3)
        restpos *= scale[:, np.newaxis, :]
        
        restpos += mean
        
        restpos = restpos.transpose(1,0) # (V, N, 3)
