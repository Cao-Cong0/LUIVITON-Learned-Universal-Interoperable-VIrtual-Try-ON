...
from src.custom_unet_2d_condition import CustomUNet2DConditionModel
...
custom_unet = CustomUNet2DConditionModel.from_config(pipe.unet.config)
custom_unet.load_state_dict(pipe.unet.state_dict())
pipe.unet = custom_unet.to(device=pipe.unet.device, dtype=pipe.unet.dtype)


syncmvd = StableSyncMVDPipeline(**pipe.components)


result_tex_rgb, textured_views = syncmvd(
	prompt=opt.prompt,
