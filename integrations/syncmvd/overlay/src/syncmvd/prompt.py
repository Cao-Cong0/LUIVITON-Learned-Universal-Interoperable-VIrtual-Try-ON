...
direction_names = ["", "front", "side", "back", "top", "bottom", 'left', 'right']
...
def azim_prompt(embeddings, pose):
    ...
    # Normalize azimuth angle to be within [0, 360]
    azim = azim % 360
    ...
    if elev > 30:
        ...
        # Assigning directions with equal azimuth ranges
        if 60 <= azim < 120:
            pos_z = embeddings["right"]
        elif 120 <= azim < 240:
            pos_z = embeddings["back"]
        elif 240 <= azim < 300:
            pos_z = embeddings["left"]
        else:
            pos_z = embeddings["front"]
    return pos_z
...
def azim_neg_prompt(embeddings, pose):
    ...
    # Assign opposite directions based on azimuth
    if 60 <= azim < 120:
        pos_z = embeddings["left"]  # Right's opposite is left
    elif 120 <= azim < 240:
        pos_z = embeddings["front"] # Back's opposite is front
    elif 240 <= azim < 300:
        pos_z = embeddings["right"] # Left's opposite is right
    else:
        pos_z = embeddings["back"]  # Front's opposite is back
    return pos_z
