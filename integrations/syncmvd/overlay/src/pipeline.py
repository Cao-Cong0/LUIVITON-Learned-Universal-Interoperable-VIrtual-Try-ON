...
from .utils import *
from .custom_unet_2d_condition import CustomUNet2DConditionModel
...
@torch.no_grad()
def get_conditioning_images(camera_poses, uvp, output_size, render_size=512, blur_filter=5, cond_type="normal"):
	...
	return conditional_images, masks, verts.cpu().numpy()


@torch.no_grad()
def export_point_cloud_with_normals(verts, normals, file_path):
    """
    Exports a point cloud with normals to a PLY file.

    Args:
        verts (torch.Tensor or np.ndarray): Vertex positions (B, N, 3).
        normals (torch.Tensor or np.ndarray): Normals (B, N, 3).
        file_path (str): File path to save the point cloud (e.g., 'output.ply').
    """
    # Ensure verts and normals are numpy arrays
    ...
    # Flatten and concatenate verts and normals
    point_cloud_data = np.concatenate([verts, normals], axis=-1)

    # Create PLY header
    header = f"""ply
				format ascii 1.0
				element vertex {len(point_cloud_data)}
				property float x
				property float y
				property float z
				property float nx
    ...
    with open(file_path, 'w') as file:
        file.write(header)
        np.savetxt(file, point_cloud_data, fmt="%.6f")
...
class StableSyncMVDPipeline(StableDiffusionControlNetPipeline):
	...
	def initialize_pipeline(
		...
		if top_cameras:
			base = len(camera_azims)
			for i, azim in enumerate(camera_azims):
				if azim < 0:
					azim += 360
				self.camera_poses.append((30, azim+15))
				self.attention_mask.append([(base+i-2)%base, (base+i-1)%base, i, (i+1)%base, (i+2)%base])
			for i, azim in enumerate(camera_azims):
				if azim < 0:
					azim += 360
				self.camera_poses.append((15, azim))
		...
		if mesh_path.lower().endswith(".obj"):
			self.mesh_center, self.mesh_scale = self.uvp.load_mesh(mesh_path, scale_factor=mesh_transform["scale"] or 1, autouv=mesh_autouv)
			# save the mesh center and scale into the result dir as mesh transformation
			mesh_transformation = {
				"center": self.mesh_center.cpu().numpy(),  # Convert tensor to NumPy array
				"scale": self.mesh_scale
			}
			np.save(f"{self.result_dir}/mesh_transform.npy", mesh_transformation)
		...
		self.uvp.set_cameras_and_render_settings(self.camera_poses, centers=camera_centers, camera_distance=3)
		...
		self.uvp_rgb.set_cameras_and_render_settings(self.camera_poses, centers=camera_centers, camera_distance=3)
	...
	def __call__(
		...
		conditioning_images, masks, _ = get_conditioning_images(self.camera_poses, self.uvp, height, render_size=height, cond_type=cond_type)
		# Feature extraction uses a 480x480 RGB/unprojection grid,
		# independently of the 512x512 diffusion input.
		projection_geometry = self.uvp.render_geometry(image_size=render_rgb_size)
		verts = projection_geometry[0].cpu().numpy()
		del projection_geometry
			# Export the vertices used to project image features back to the mesh.

		# # np.save(f"{self.result_dir}/verts.npy", verts)
		# # Separate positions (XYZ) and depth (last component)
		# verts_pos = verts[..., :3]  # Shape (24, 480, 480, 3)
		# verts_depth = verts[..., 3]  # Shape (24, 480, 480)

		# # Apply scaling
		# verts_pos = verts_pos * (1 / self.mesh_scale)

		# # Apply translation (broadcasting to match verts_pos shape)
		# verts_pos = verts_pos + self.mesh_center.cpu().numpy()

		# # Reconstruct the verts array with the same shape
		# verts = np.concatenate([verts_pos, verts_depth[..., None]], axis=-1)  # Shape (24, 480, 480, 4)

		np.save(f"{self.result_dir}/verts.npy", verts)

		# # export verts_pos to a ply file using trimesh
		# mesh = trimesh.Trimesh(vertices=verts_pos.reshape(-1, 3), process=False)
		# mesh.export(f"{self.result_dir}/point_cloud.ply")
		...
		extract_layer = False
		f_map_iters = 0
		# f_map = torch.zeros((len(self.camera_poses),1280,32,32)).half().to(device) # float16
		f_map0 = torch.zeros((len(self.camera_poses),1280,int(height/32),int(height/32))).to(device) # float32
		f_map1 = torch.zeros((len(self.camera_poses),1280,int(height/16),int(height/16))).to(device) # float32
		f_map2 = torch.zeros((len(self.camera_poses),640,int(height/8),int(height/8))).to(device)


		# aggregation_steps = (num_inference_steps*3)//4
		aggregation_steps = (num_inference_steps*3)//5
		f_map_weights = torch.linspace(0.1, 1, steps=aggregation_steps).to(device)
		# aggregation_steps = (num_inference_steps*2)//4
		# f_map_weights = torch.linspace(0.5, 1, steps=aggregation_steps).to(device)
		# aggregation_steps = (num_inference_steps*3)//4
		# f_map_weights = torch.linspace(0.3, 0.95, steps=aggregation_steps).to(device)
			...
			for i, t in enumerate(timesteps):
				...
				for prompt_tag, prompt_embeds in prompt_embeds_groups.items():
					...
					for model_input_batch, prompt_embeds_batch, down_block_res_samples_batch, mid_block_res_sample_batch, meta \
						...
						if i >= (num_inference_steps - aggregation_steps):
							extract_layer = True
						...
						if extract_layer and prompt_tag == "positive":
							f_maps = noise_pred[1] # a list with length=4, with shape = [[10, 1280, 16, 16], [10, 1280, 32, 32], [10, 640, 64, 64], [10, 320, 64, 64]]
							# note that the difination of f_map is around line 492, which is needed to be changed according to shapes from different layers
							f_map0 += torch.nn.functional.normalize(f_maps[0], dim=1)*f_map_weights[f_map_iters] # shape = [10, 1280, 16, 16]
							f_map1 += torch.nn.functional.normalize(f_maps[1], dim=1)*f_map_weights[f_map_iters] # shape = [10, 1280, 32, 32]
							f_map2 += torch.nn.functional.normalize(f_maps[2], dim=1)*f_map_weights[f_map_iters] # shape = [10, 640, 64, 64]
							f_map_iters += 1
						if i == (num_inference_steps - 1) and prompt_tag == "positive":
							f_map0_float32 = f_map0.to(torch.float32)
							f_map1_float32 = f_map1.to(torch.float32)
							f_map2_float32 = f_map2.to(torch.float32)

							np.savez(
										f'{self.result_dir}/f_maps.npz',
										f_map0=f_map0_float32.detach().cpu().numpy(),
										f_map1=f_map1_float32.detach().cpu().numpy(),
										f_map2=f_map2_float32.detach().cpu().numpy()
									)
							# if we want to read and transform to bf16 tensor, use torch.from_numpy(np.load('f_map_bf16.npy')).to(device).half()
						noise_pred = noise_pred[0]
						noise_pred_list.append(noise_pred) # noise_pred.shape = [10, 4, 96, 96], which is [nv, rgba, latent_size, latent_size]
		...
		for i, view in enumerate(textured_views):
			# Permute the tensor to match (H, W, C) format and convert it to a numpy array
			view_rgb = view[:3].permute(1, 2, 0).cpu().numpy()  # Take only the RGB channels (first 3 channels)

			pil_image = numpy_to_pil(view_rgb)[0]  # Assuming numpy_to_pil returns a list of images

			# Save the image with the appropriate filename
			# create a directory to save the images
			rgb_folder = f"{self.result_dir}/rgb_views"
			# create the directory if it does not exist
			if not os.path.exists(rgb_folder):
				os.makedirs(rgb_folder)
			pil_image.save(f"{rgb_folder}/{i}.jpg")
		# display(v)
