...
from pytorch3d.renderer import BlendParams, AmbientLights, Materials
...
class HardNChannelFlatShader(ShaderBase):
	...
	def __init__(
		...
		# Ensure lights have correct number of channels
		if not isinstance(lights, AmbientLights) or not lights.ambient_color.shape[-1] == channels:
			lights = AmbientLights(
				ambient_color=ones,
				device=device,
		...
		# Ensure materials have correct number of channels
		if not materials or not materials.ambient_color.shape[-1] == channels:
			materials = Materials(
				device=device,
				diffuse_color=zeros,
		...
		# Set the background color in blend_params to black
		blend_params_new = BlendParams(background_color=(0.0,) * channels)
		if not isinstance(blend_params, BlendParams):
			blend_params = blend_params_new
		else:
			background_color_ = blend_params.background_color
	...
	def forward(self, fragments: Fragments, meshes: Meshes, **kwargs) -> torch.Tensor:
		...
		# Set background pixels to black using hard_rgb_blend
		images = hard_rgb_blend(colors, fragments, blend_params)
		return images
